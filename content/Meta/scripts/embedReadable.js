...
\`\`\``;
  ...
  const isDriveFile = articleURL.includes("drive.google.com") && 
                    (articleURL.includes("/file/d/") || articleURL.includes("open?id="));
...
\`\`\``;
  ...
  if (isDriveFile) {
    ...
    const fileId = articleURL.includes("/file/d/")
      ? articleURL.split("/file/d/")[1].split("/")[0]  // Handles /file/d/ format
      : articleURL.split("id=")[1].split("&")[0];      // Handles open?id= format

    // 2. Build the content using the extracted fileId
    frontmatter = `---
tags:
- resources/articles
